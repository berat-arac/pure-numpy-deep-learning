...
from puredl.text import LSTM
from puredl.utils.gradcheck import finite_difference_gradient, relative_error


def test_lstm_bptt_matches_finite_difference():
    rng = np.random.default_rng(3)
    layer = LSTM(2, 2, rng=rng, dtype=np.float64)
    x = rng.normal(size=(1, 2, 2)) * 0.15
    upstream = rng.normal(size=(1, 2, 2)) * 0.2
    layer.forward(x)
    grad_x, _, _ = layer.backward(upstream)
    ...
    assert relative_error(analytic_wih, numeric_wih) < 2e-6
    assert relative_error(analytic_whh, numeric_whh) < 2e-6
